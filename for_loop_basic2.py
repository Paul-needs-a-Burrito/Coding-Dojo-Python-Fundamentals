...
def count_positives(llist):
    counter = 0
    for x in range(len(llist)):
        if llist[x] > 0:
            counter += 1

    llist[len(llist) - 1] = counter
    return llist
...
def ultimate_analysis(llist):
    ...
    dic = {'sumTotal': sum, 'average': (sum / length), 'minimum': min, 'maximum': max, 'length': length}
...
def reverse_list(llist):
    for x in range( int( (len(llist) / 2) )  ):
        temp = llist[x]
        llist[x] = llist[len(llist) - (1 + x)]
        llist[len(llist) - (1 + x)] = temp
